...
A CADET-RDM repository typically consists of a `project repository` which contains the code and an `output repository` which contains the results generated by the project code. By initializing an RDM repository the necessary file structure for tracking and version-controlling the project and output will be created. 
...
A
[GitLab Personal Access Token (PAT)](https://docs.gitlab.com/ee/user/profile/personal_access_tokens.html) or [GitHub PAT](https://github.com/settings/tokens?type=beta) with API access rights have to be created
